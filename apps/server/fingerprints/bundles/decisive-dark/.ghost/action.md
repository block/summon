...
*is* the surface's single [accent moment](accent-moment) — a *Send* on a screen where
...
result. Restraint here is what makes the one settle-in on the [answer](answer)
read as meaningful rather than as one effect among many.

The action stack follows the [one-sentence meaning](meaning) and resolves the
[answer](answer) into a move the reader can make — it is where the surface's momentum
lands, and where the single [accent moment](accent-moment) may be spent if the action
