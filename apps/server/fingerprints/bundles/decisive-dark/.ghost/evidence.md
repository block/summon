...
evidence region per surface, suppressed entirely when the [answer](answer) already
...
[accent moment](accent-moment); otherwise bars stay `--dd-bar-neutral`.
...
Evidence justifies the [answer](answer) rather than postponing it, sits beneath the
hero on near-black tiers, and hands off to the [one-sentence meaning](meaning) that
names the so-what before the [action stack](action) guides the next move.
