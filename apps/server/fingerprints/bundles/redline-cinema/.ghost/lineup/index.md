...
A lineup invites browsing across a family. Reach for this surface when the user's
first question is "what range exists, and which part draws me in?" — a model
family, a collection, a season of sessions, a set of chapters — not when the job
is to reveal one thing (the [launch](../launch)), to prove performance data (the
[spec](../spec)), to transact across dense listings (the [catalog](../catalog)),
or to find a location or service (the [directory](../directory)).

**Lineup category browse.** Present image-led category plates for product
families, collections, sessions, or chapters, with sparse uppercase (optionally
numbered) labels, delaying dense filters until after the first image-led browse
