...
[cinematic image](cinematic-image) hands the reader forward. No crossfade-to-white
...
cadence system enforces the scarcity that the [controls](controls) and
[spec system](spec-system) each spend locally.
...
This cadence paces the [cinematic image](cinematic-image) breakouts and the
[hairline and brightness depth](depth) rhythm into numbered acts, gives the
[large-number spec system](spec-system) its mono data voice, and holds the
[scarce-red controls](controls) to one voltage spend per viewport; the
[single red livery band](livery-band) remains the lone moment allowed to break the
