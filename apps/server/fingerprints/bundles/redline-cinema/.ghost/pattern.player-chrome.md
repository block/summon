...
placeholder discipline](pattern.cinematic-image) applies unchanged.

The player dresses footage inside the [cinematic image](pattern.cinematic-image) frame,
borrows its mono data voice and reveal restraint from the [editorial
cadence](pattern.editorial-cadence), and holds the same red scarcity as the [control
system](pattern.controls): the one voltage spend per viewport is never duplicated by
player chrome.

Related: reinforces `pattern.cinematic-image`, `pattern.controls`, `pattern.editorial-cadence`.
