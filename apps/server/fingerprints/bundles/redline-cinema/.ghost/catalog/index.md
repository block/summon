...
A catalog lists and transacts. Reach for this surface when the user's first
question is "what is available, and how do I act on it?" — a product listing,
inventory, pricing, configuration, or a form-heavy transactional region — not when
the job is to reveal one thing (the [launch](../launch)), to prove performance
data (the [spec](../spec)), to find a location or service (the
[directory](../directory)), or to browse a product family (the
[lineup](../lineup)).
...
labels, hairline borders, one red fill per region, disciplined filter rows that
stack cleanly on mobile. Listings stay dense enough to act while remaining
visually connected to the cinematic editorial frame.
