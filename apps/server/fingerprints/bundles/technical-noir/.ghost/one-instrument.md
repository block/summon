...
[density field](density-field)), not a chart, a gradient, or a gauge; when a
...
of [tiles, rows](tiles), and [evidence panels](terminal-evidence) without
...
This discipline governs how the [tile and row system](tiles) keeps a long page
reading as one workspace, how the [terminal evidence panels](terminal-evidence)
stay plain outside the primary reading path, and how the
[control system](controls) spends its one off-white primary action — the same
