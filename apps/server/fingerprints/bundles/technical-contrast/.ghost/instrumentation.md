...
[contrast-plane](contrast-planes) flip via `--contrast-plane-invert` is the
...
one all-caps [mono label](mono-labels) voice — never a decorative ticker, fake
telemetry, or animated chrome.

**Exposed grid as credibility device.** In exactly one deliberate location per
surface — typically behind the primary [data sheet](data-sheets) — render the
...
table overview in the [data-sheet](data-sheets) anatomy. Same entities, same
...
callouts) live in the [drafting-mark system](drafting-marks) and follow the same
...
The metadata strip and anchor speak in the [mono label](mono-labels) voice; the
grid exposure and index/overview duality serve the [data sheets](data-sheets);
and the narrative-beat rule governs when the [contrast planes](contrast-planes)
