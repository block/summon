...
[mono metadata pills](type-system) would otherwise crowd.
...
tears, and the [stream cadence](cadence) governs when a feed has earned one.
...
type system](type-system) — same whisper register, more structure; catalog cards
and opening plates sit flat on the [tile system](tiles) rails under the same
...
[dark canvas and hazard-accent system](canvas) — the grammar adds filing and
