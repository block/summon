...
export function DiagnosticsDock({
  ...
  const rootClass = embedded
    ...
    : cn(pageWidthClass, panelClass, 'mt-9 flex max-h-[min(720px,calc(100vh-180px))] min-h-0 flex-col overflow-hidden');
  const panelClassName = 'min-h-0 flex-1 overflow-auto bg-surface';
  const logClassName = 'max-h-none min-h-0 h-full';

  return (
    <section className={rootClass} aria-label="Diagnostics">
      <div className="shrink-0 flex flex-wrap gap-1.5 border-b border-line bg-surface p-2.5" role="tablist" aria-label="Diagnostics tabs">
      ...
      <div className={panelClassName} id="diagnostics-stream" data-diagnostics-panel="stream" hidden={diagnosticsTab !== 'stream'}>
        <LogView id="log" className={logClassName}>
          {logs.map((entry, index) => <div key={index} className={logToneClass(entry.cls)}>{entry.text}</div>)}
        </LogView>
      </div>
      <div className={panelClassName} id="diagnostics-devtools" data-diagnostics-panel="devtools" hidden={diagnosticsTab !== 'devtools'}>
        <LogView id="devtools-log" className={logClassName}>
      ...
      <div className={panelClassName} id="diagnostics-timing" data-diagnostics-panel="timing" hidden={diagnosticsTab !== 'timing'}>
