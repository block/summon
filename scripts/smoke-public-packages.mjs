...
await writeFile(join(projectDir, 'smoke.mjs'), [
  ...
  "import { compileSystemContracts, deriveSurfacePlanControls, parseProtocolLine, StreamGraph } from '@anarchitecture/summon/engine';",
  ...
  "import { runSurfaceGeneration, resolveSurfaceGenerationPlan, summarizeContractIssues } from '@anarchitecture/summon-server';",
  ...
  "if (typeof runSurfaceGeneration !== 'function' || typeof resolveSurfaceGenerationPlan !== 'function' || typeof summarizeContractIssues !== 'function') throw new Error('server import failed');",
  ...
  "}",
  "for (const forbidden of ['buildEditBlock', 'generateSurfaceStream']) {",
